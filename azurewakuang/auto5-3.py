...
for x in ['1','2','3','4','5','6']:
    ...
    if '1' in x: 
        ...
        for i in range(10, -1, -1):
            print("\r稍后将运行第二遍脚本(查漏补缺)，还需等待{}秒".format(i), end="", flush=True)
    if '2' in x: 
        print("\n------------------------------------------------------------------------------\n")
        ...
        print("创建资源组成功")
        for i in range(10, -1, -1):
            print("\r稍后将切换订阅运行第三遍脚本，还需等待{}秒".format(i), end="", flush=True)
    if '3' in x: 
        print("\n------------------------------------------------------------------------------\n")
        print("以下是已创建的虚拟机列表：")
        get_default_cli().invoke(['vm', 'list', '--query', '[*].name'])
        print("\n\n-----------------------------------------------------------------------------\n")
        for i in range(10, -1, -1):
            print("\r稍后将运行第四遍脚本(查漏补缺)，还需等待{}秒".format(i), end="", flush=True)
    if '4' in x: 
        print("\n------------------------------------------------------------------------------\n")
        print("大功告成！在31个区域创建虚拟机的命令已成功执行")
        for i in range(60, -1, -1):
        ...
        print("创建资源组成功")
        for i in range(10, -1, -1):
            print("\r稍后将切换订阅运行第五遍脚本，还需等待{}秒".format(i), end="", flush=True)
    if '5' in x: 
        print("\n------------------------------------------------------------------------------\n")
        print("以下是已创建的虚拟机列表：")
        get_default_cli().invoke(['vm', 'list', '--query', '[*].name'])
        print("\n\n-----------------------------------------------------------------------------\n")
        for i in range(10, -1, -1):
            print("\r稍后将运行第六遍脚本(查漏补缺)，还需等待{}秒".format(i), end="", flush=True)
    if '6' in x: 
        print("\n------------------------------------------------------------------------------\n")
        print("以下是已创建的虚拟机列表：")
        get_default_cli().invoke(['vm', 'list', '--query', '[*].name'])
